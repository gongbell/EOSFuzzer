...
#define GOTO(offset) (pc = &istream[offset], instrument.append("Goto(" + std::to_string(uint64_t(pc)) + ")"))
...
Result Thread::Unop(UnopFunc<R, T> func) {
  auto value = PopRep<T>();
  instrument.append("(" + std::to_string(value) + ")");
  auto result_value = func(value);
  instrument.append("(" + std::to_string(result_value) + ")\n");
...
Result Thread::Binop(BinopFunc<R, T> func) {
  ...
  instrument.append("(" + std::to_string(lhs_rep) + "," + std::to_string(rhs_rep) + ")");
  auto result_value = func(lhs_rep, rhs_rep);
  instrument.append("(" + std::to_string(result_value) + ")\n");
  return PushRep<R>(result_value);
}
...
#define Unop(...) ((instrument.append(#__VA_ARGS__)), Unop(__VA_ARGS__))
#define UnopTrap(...) ((instrument.append(#__VA_ARGS__)), UnopTrap(__VA_ARGS__))
#define Binop(...) ((instrument.append(#__VA_ARGS__)), Binop(__VA_ARGS__))
#define BinopTrap(...) ((instrument.append(#__VA_ARGS__)), BinopTrap(__VA_ARGS__))

Result Thread::Run(int num_instructions) {

  uint64_t runStartTime = std::chrono::duration_cast<std::chrono::nanoseconds>(
    std::chrono::system_clock::now().time_since_epoch()
  ).count();
  instrument.clear();
  std::ofstream f(std::string(getenv("HOME")) + "/.local/share/eosio/op.txt", std::ios::app);
  f << "Run! " << runStartTime << std::endl;
  //std::set<uint64_t> runCoverageSet;
  ...
  for (int i = 0; i < num_instructions; ++i) {
    ...
    //runCoverageSet.insert(uint64_t(pc));
    ...
    switch (opcode) {
      ...
      case Opcode::BrIf: {
        ...
        instrument.append("BrIf(");
        if (Pop<uint32_t>()) {
          instrument.append("TRUE,");
          GOTO(new_pc);
        }
        else {
          instrument.append("FALSE,Goto(" + std::to_string(uint64_t(new_pc)) + ")");
        }
        instrument.append(")\n");
        break;
      }
      ...
      case Opcode::BrTable: {
        ...
        instrument.append("BrTable(");
        instrument.append(std::to_string(num_targets) + ",");
        instrument.append(std::to_string(table_offset) + ",");
        instrument.append(std::to_string(key) + ",");
        instrument.append(std::to_string(uint64_t(entry)) + ",");
        instrument.append(std::to_string(drop_count) + ",");
        instrument.append(std::to_string(keep_count) + ",");
        GOTO(new_pc);
        instrument.append(")\n");
        break;
      }
      ...
      case Opcode::Return:
        if (call_stack_top_ == 0) {
          ...
          instrument.append("Return(StackEmpty)\n");
          goto exit_loop;
        }
        instrument.append("Return(");
        GOTO(PopCall());
        instrument.append(")\n");
        break;
      ...
      case Opcode::CallIndirect: {
        ...
        instrument.append("CallIndirect(");
        if (func->is_host) {
          instrument.append("isHost,TODO:UNDEFINED");
          CHECK_TRAP(CallHost(cast<HostFunc>(func)));
        } else {
          instrument.append("isNotHost,");
          CHECK_TRAP(PushCall(pc));
...
#undef ATOMIC_RMW
      ...
      case Opcode::InterpBrUnless: {
        IstreamOffset new_pc = ReadU32(&pc);
        instrument.append("InterpBrUnless(");
        if (!Pop<uint32_t>()) {
          instrument.append("FALSE,");
          GOTO(new_pc);
        }
        else {
          instrument.append("TRUE,Goto(" + std::to_string(uint64_t(new_pc)) + ")");
        }
        instrument.append(")\n");
        break;
      }
...
exit_loop:
  ...
  /* instrument */
  f << instrument;
  uint64_t runEndTime = std::chrono::duration_cast<std::chrono::nanoseconds>(
    std::chrono::system_clock::now().time_since_epoch()
  ).count();
  f << "End! " << runEndTime << std::endl;
  f.close();

  /*
  test data vector
  */
  /*std::ofstream ftmp(std::string(getenv("HOME"))
    + "/.local/share/eosio/data_" + std::to_string(env()->istream_->data.size())
    + ".bin");
  ftmp.write(static_cast<char*>(static_cast<void*>(env()->istream_->data.data())), env()->istream_->data.size());
  ftmp.close();*/

  /*std::ofstream fRunCoverage(std::string(getenv("HOME"))
    + "/.local/share/eosio/cov_" + std::to_string(env()->istream_->data.size()) + "_"
    + std::to_string(runStartTime) + "_" 
    + std::to_string(runEndTime) + ".bin");
  uint64_t sizeOfCov = runCoverageSet.size();
  uint64_t* covArray = new uint64_t[sizeOfCov];
  int ptrCovArray = 0;
  for(std::set<uint64_t>::iterator it = runCoverageSet.begin(); it != runCoverageSet.end(); ++it) {
    covArray[ptrCovArray++] = *it;
  }
  #define FRUNCOVERAGE_WRITE(a, b, c) fRunCoverage.write(static_cast<char*>(static_cast<void*>(a)), sizeof(b) * c)
  //FRUNCOVERAGE_WRITE(&runStartTime, runStartTime, 1);
  //FRUNCOVERAGE_WRITE(&runEndTime, runEndTime, 1);
  //FRUNCOVERAGE_WRITE(&sizeOfCov, sizeOfCov, 1);
  FRUNCOVERAGE_WRITE(covArray, uint64_t, sizeOfCov);
  #undef FRUNCOVERAGE_WRITE
  fRunCoverage.close();
  delete[] covArray;*/
