...
class console_api : public context_aware_api {
   public:
      ...
      /* instrument */
      void logfile(null_terminated_ptr str) {
         uint64_t nowInNanoseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(
            std::chrono::system_clock::now().time_since_epoch()
         ).count();
         //printui(nowInNanoseconds);
         //prints(str);
         std::ofstream f(std::string(getenv("HOME")) + "/.local/share/eosio/logfile.txt", std::ios::app);
         f << nowInNanoseconds << ' ' << (const char*)str << std::endl;
         f.close();
      }
...
class context_free_transaction_api : public context_aware_api {
   public:
      ...
      int tapos_block_num() {
        ...
        std::ofstream f0(std::string(getenv("HOME")) + "/.local/share/eosio/logfile.txt", std::ios::app);
        f0 << nowInNanoseconds << ' ' << "tapos_block_num()" << std::endl;
        f0.close();
        std::ofstream f(std::string(getenv("HOME")) + "/.local/share/eosio/func.txt", std::ios::app);
        f << "tapos_block_num()" << std::endl;
        f.close();
      ...
      int tapos_block_prefix() {
        ...
        std::ofstream f0(std::string(getenv("HOME")) + "/.local/share/eosio/logfile.txt", std::ios::app);
        f0 << nowInNanoseconds << ' ' << "tapos_block_prefix()" << std::endl;
        f0.close();
        std::ofstream f(std::string(getenv("HOME")) + "/.local/share/eosio/func.txt", std::ios::app);
        f << "tapos_block_prefix()" << std::endl;
        f.close();
